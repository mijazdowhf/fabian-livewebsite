...
class UserController extends Controller
{
    ...
    public function myApplications()
    {
        ...
        $mortgages = \App\Models\SellingMortgageApplication::where('user_id', $user->id)
            ->with('agent')
            ->orderBy('created_at', 'desc')
            ->get();

        $personalLoans = \App\Models\LoanInquiry::where('user_id', $user->id)
            ->with('agent')
            ->orderBy('created_at', 'desc')
            ->get();

        // Employee loans still fetched for modals (in case they exist)
        $employeeLoans = \App\Models\EmployeeLoanApplication::where('user_id', $user->id)
            ->with('agent')
            ->orderBy('created_at', 'desc')
            ->get();

        return view('Template::user.applications.index', compact(
            'pageTitle',
            'mortgages',
            'personalLoans',
            'employeeLoans'
